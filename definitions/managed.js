...
ctx =>`
    ...
    FROM 
        ${ctx.ref('GF_ext_clientes2')} cl
    JOIN 
        ${ctx.ref('GF_ext_enderecos2')} ad
        ON cl.cliente_cpf = ad.cliente_cpf
    JOIN 
        ${ctx.ref('GF_ext_itens_produtos_pedidos2')} it
        ON cl.cliente_cpf = it.pedido_cliente_cpf
    `)


transformation.transform(
    'GF_tb_itens_pedidos_clientes2', 
    'managed', 
    'table', 
    ['managed'], 
    ['GF_ext_clientes2', 'GF_ext_enderecos2', 
    'GF_ext_itens_produtos_pedidos2'], 
    query
)
